module "asg_web_blu" {
  ...
  ami_id               = "${var.web_asg_ami_blu[var.region]}"
  ...
  user_data        = "${var.userdata_file_web_blu}"
  name             = "${var.name_prefix}-${terraform.workspace}-web-blu"
  asg_max_size     = "${var.web_asg_max[terraform.workspace]}"
  asg_min_size     = "${var.web_asg_min[terraform.workspace]}"
  desired_capacity = "${var.web_asg_des_blu}"

  #target_group_arns    = ["${var.alb_tg_arn}"]
  alb_target_group_arn = "${data.aws_lb_target_group.blu.arn}"
  ...
  app_role    = "web-server"
  app_id      = "web"
  app_version = "${var.app_version_web_blu}"
  svr_type    = "bg"
  offhours    = "off"
}
